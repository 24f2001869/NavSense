"""
SIH26168 - Stage C8-7D: Component Attribution and Ablation Audit
Module: experiments/run_ablation_audit_c8_7d.py

Performs a rigorous, systematic component-by-component on/off ablation audit across:
- Baseline ESKF (Pure strapdown IMU alone, no speed, no compass, no NHC, no ZUPT, no map)
- +RF forward speed alone
- +Compass alone
- +NHC alone
- +ZUPT alone
- +Map alone
- Progressive step-up additions (Speed -> Compass -> NHC -> ZUPT -> Map)
- Leave-one-out ablations from the Full Smartphone Stack (-Speed, -Compass, -NHC, -ZUPT, -Map)

Evaluates on identical outage windows across 5s, 10s, 20s, 30s, 60s on:
- Vta04 (Untouched Highway Test Benchmark)
- Vta02 (Clean Suburban Benchmark)

Strict Deployment Boundary:
- Zero vehicle CAN, wheel speed, or steering signals used in smartphone conditions.
- Vehicle-side signals used strictly as offline evaluation reference.
"""
...
def prepare_trip_ablation_data(trip_name: str, rf_model: RandomForestRegressor, dt: float = 0.1) -> Dict[str, Any]:
    """Ingests trip telemetry, OSM map, extracts causal ML speed, and precomputes series."""
    print(f"[{trip_name}] Loading trip data for ablation audit...", flush=True)
...
def simulate_ablation_window(
    ...
    """
    Simulates a GNSS blackout window with strictly decoupled boolean switches:
    flags:
      use_speed: bool (1-DOF forward velocity update via ML speed)
      use_compass: bool (confidence-gated calibrated magnetometer update)
      use_nhc: bool (lateral and vertical zero-velocity constraints)
      use_zupt: bool (stationary zero-velocity updates)
      use_map: bool (MHT road map snapping)
      use_can_ref: bool (substitutes CAN wheel speed for phone ML speed)
    """
    ...
    for step_k in range(kw, kw + w_dur):
        ...
        # 2. Forward Velocity Update (1-DOF Forward Velocity ONLY)
        if flags.get('use_speed', False):
            if flags.get('use_can_ref', False):
                cur_wheel_spd = float(wheel_speed[step_k])
                can_wheel_fusion.update_eskf_forward_velocity(eskf, cur_wheel_spd, apply_nis_gate=True)
            else:
                cur_speed_est = float(ml_speed[step_k])
                phone_speed_fusion.update_eskf_forward_velocity(eskf, cur_speed_est, apply_nis_gate=True)

        # 3. Non-Holonomic Constraints (NHC lateral and vertical zero velocity)
        if flags.get('use_nhc', False):
            nhc.update_eskf(eskf)

        # 4. Confidence-Gated Compass Heading Update
        ...
        # 6. Map Matching Constraint (1.0s interval)
        if map_mgr is not None and (t_now - last_map_time >= 0.999):
            last_map_time = t_now
            spd_for_map = float(wheel_speed[step_k]) if flags.get('use_can_ref', False) else float(ml_speed[step_k])
        ...
        # Trajectory-aligned along-track / cross-track error
...
def run_full_ablation_audit():
    """Executes the master Stage C8-7D component attribution and ablation study."""
    print("=================================================================", flush=True)
    print("STAGE C8-7D: COMPONENT ATTRIBUTION AND ABLATION AUDIT", flush=True)
    print("=================================================================", flush=True)

    print("Training Causal Random Forest speed model on Vta02 train set...", flush=True)
    ...
    d_vta02 = prepare_trip_ablation_data('Vta02', rf_model)
    d_vta04 = prepare_trip_ablation_data('Vta04', rf_model)

    horizons = [5, 10, 20, 30, 60]

    # Explicit condition configuration dictionary
    CONDITIONS = {
        # A. Baseline
        'A0_Baseline_PureIMU': {
            'use_speed': False, 'use_compass': False, 'use_nhc': False, 'use_zupt': False, 'use_map': False, 'use_can_ref': False,
            'desc': 'Pure Strapdown IMU Dead Reckoning (No constraints)'
        },
        # B. Single-Component Additions from Baseline
        'B1_Speed_Only': {
            'use_speed': True, 'use_compass': False, 'use_nhc': False, 'use_zupt': False, 'use_map': False, 'use_can_ref': False,
            'desc': 'Baseline + RF Forward Speed (1D)'
        },
        'B2_Compass_Only': {
            'use_speed': False, 'use_compass': True, 'use_nhc': False, 'use_zupt': False, 'use_map': False, 'use_can_ref': False,
            'desc': 'Baseline + Calibrated Gated Compass'
        },
        'B3_NHC_Only': {
            'use_speed': False, 'use_compass': False, 'use_nhc': True, 'use_zupt': False, 'use_map': False, 'use_can_ref': False,
            'desc': 'Baseline + Non-Holonomic Constraints (Lateral & Vertical)'
        },
        'B4_ZUPT_Only': {
            'use_speed': False, 'use_compass': False, 'use_nhc': False, 'use_zupt': True, 'use_map': False, 'use_can_ref': False,
            'desc': 'Baseline + Zero Velocity Updates (Stationary detector)'
        },
        'B5_Map_Only': {
            'use_speed': False, 'use_compass': False, 'use_nhc': False, 'use_zupt': False, 'use_map': True, 'use_can_ref': False,
            'desc': 'Baseline + MHT Road Map Snapping'
        },
        # C. Progressive Step-Up Chain
        'C1_Speed_Compass': {
        ...
        # D. Leave-One-Out Ablations (from Full Smartphone Stack)
        'D1_Ablate_Minus_Speed': {
            'use_speed': False, 'use_compass': True, 'use_nhc': True, 'use_zupt': True, 'use_map': True, 'use_can_ref': False,
            'desc': 'Full Stack MINUS Speed (Compass + NHC + ZUPT + Map)'
        },
        'D2_Ablate_Minus_Compass': {
            'use_speed': True, 'use_compass': False, 'use_nhc': True, 'use_zupt': True, 'use_map': True, 'use_can_ref': False,
            'desc': 'Full Stack MINUS Compass (Speed + NHC + ZUPT + Map)'
        },
        'D3_Ablate_Minus_NHC': {
            'use_speed': True, 'use_compass': True, 'use_nhc': False, 'use_zupt': True, 'use_map': True, 'use_can_ref': False,
            'desc': 'Full Stack MINUS NHC (Speed + Compass + ZUPT + Map)'
        },
        'D4_Ablate_Minus_ZUPT': {
            'use_speed': True, 'use_compass': True, 'use_nhc': True, 'use_zupt': False, 'use_map': True, 'use_can_ref': False,
            'desc': 'Full Stack MINUS ZUPT (Speed + Compass + NHC + Map)'
        },
        'D5_Ablate_Minus_Map': {
            'use_speed': True, 'use_compass': True, 'use_nhc': True, 'use_zupt': True, 'use_map': False, 'use_can_ref': False,
            'desc': 'Full Stack MINUS Map (Speed + Compass + NHC + ZUPT)'
        },
        # E. External Vehicle CAN Reference
        'REF_CAN_Wheel_Full': {
    ...
    for trip_data, tname in [(d_vta04, 'Vta04'), (d_vta02, 'Vta02')]:
        print(f"\n=================================================================", flush=True)
        print(f"RUNNING ABLATION SUITE ON: {tname}", flush=True)
        print(f"=================================================================", flush=True)
        ...
        for dur_s in horizons:
            ...
            for c_key, c_cfg in CONDITIONS.items():
                ...
                for w_idx in range(n_windows):
                    ...
                    res = simulate_ablation_window(trip_data, kw, w_dur, c_cfg)
            ...
            # Print comparison for key horizons
            ...
            print("-" * 88, flush=True)
            for c_key in ['A0_Baseline_PureIMU', 'B1_Speed_Only', 'B2_Compass_Only', 'B3_NHC_Only', 'B4_ZUPT_Only',
                          'C1_Speed_Compass', 'C2_Speed_Compass_NHC', 'C3_Speed_Compass_NHC_ZUPT', 'C4_Full_Smartphone_Map',
                          'D1_Ablate_Minus_Speed', 'D2_Ablate_Minus_Compass', 'D3_Ablate_Minus_NHC', 'REF_CAN_Wheel_Full']:
        ...
        all_ablation_results['trips'][tname] = trip_summary

    # Save master structured ablation JSON
    out_file = RES_DIR / "c8_7d_ablation_audit.json"
    with open(out_file, 'w') as f:
        json.dump(all_ablation_results, f, indent=2)
    print(f"\nSaved structured ablation results to: {out_file}", flush=True)
