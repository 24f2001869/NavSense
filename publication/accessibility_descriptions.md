...
* **Short Alt-Text**: Four-panel comparative visualization contrasting held-out forward velocity estimation accuracy, integrated position drift at 30 s and 60 s, cross-route generalization across four road environments, and per-trip MAE distributions across 19 test routes between 6-trip (1.1 h) and 39-trip (12.5 h) training datasets.
* **Long Image Description**: Four-panel comparative visualization across 19 held-out test routes (115,420 prediction epochs, 240.6 km):
  - *Panel (a) (Forward Velocity MAE)*: Bar chart showing forward velocity Mean Absolute Error (MAE) dropping from 6.17 m/s (22.2 km/h) with 6 training trips down to 2.76 m/s (9.9 km/h) with 39 training trips, representing a 55.27% error reduction.
  ...
  - *Panel (c) (Cross-Route Generalisation Across Road Environments)*: Grouped bar chart depicting forward velocity MAE across four distinct driving environments: Suburban Towns (Vta, dropping from 3.82 to 2.59 m/s, a 32.2% reduction), Dense Urban (Vtb, 5.35 to 3.31 m/s, a 38.1% reduction), Mountain Routes (Vw, 8.68 to 2.50 m/s, a 71.2% reduction), and Motorways (V-Vfa, 13.18 to 3.41 m/s, a 74.1% reduction).
  - *Panel (d) (Per-Trip MAE Across 19 Held-Out Test Routes)*: Box-and-whisker plot displaying the distribution of per-trip forward velocity Mean Absolute Error (MAE) across all 19 individual held-out test routes. Expanding the training dataset from 6 to 39 trips substantially compresses the inter-quartile range, reduces median per-trip MAE from ~5.4 m/s to ~2.6 m/s, and eliminates high-error outliers.
...
* **Long Image Description**: Three-panel plot:
  ...
  - *Panel (b) (Innovation Instability Across 83 Windows)*: Normalised innovation squared ($NIS_x$) across 83 evaluation windows. Coupled NHC exhibits large turn-associated NIS spikes, with a reported mean of 82.12, whereas TCN alone (mean 6.55) and Decoupled Damping (mean 6.60) remain bounded near the empirical baseline reference level of 6.55.
...
* **Essential Reader Takeaway**: Provides no evidence that the tested 10-Hz smartphone vibration features provide a reliable forward speed signal; the persistent low-frequency peak is consistent with low-frequency vehicle-body/chassis dynamics; the physical mechanism was not directly identified.
