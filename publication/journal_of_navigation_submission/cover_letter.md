...
6. **Benchmark Reality Check:** Across 13 usable 60-second blackout test routes, adaptive fusion achieves 96.65 m mean absolute drift; the macro-average normalised drift is 16.76% across the 12 dynamic routes, with the stationary control route Vw15 excluded from Eq. (13) to avoid distortion from the near-zero travel-distance denominator. Three of 13 routes satisfy the sub-10% benchmark (23.08%), highlighting both the measured improvements and the remaining empirical limitations of standalone smartphone navigation.
